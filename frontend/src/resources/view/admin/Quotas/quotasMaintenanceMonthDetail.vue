...
const pageTitle = computed(() => {
  ...
  const label = first?.details?.[0]?.month_label ?? '';
...
const goTo = (quota) => {
  const ownerId = quota.owner_id ?? quota.details?.[0]?.departament?.owner?.id;
  if (quota.pending_validation && quota.pay_id) {
    router.push(`/admin/pay/validate/${quota.pay_id}`);
    return;
  }
  router.push(
    `/admin/quota/details/month/${quota.month}?year=${year.value}&owner=${ownerId}`
  );
};

const goToValidate = (quota, event) => {
...
const getTitleQuota = (quota) => {
  const label = quota.details?.[0]?.month_label ?? '';
  return `Mensualidad: ${label} ${year.value}`;
};
...
const getStatusInfo = (status) => {
  ...
  if (status === 2) {
    return { color: 'warning', icon: 'eva-alert-circle-outline', label: 'Pendiente de aprobación ' };
  }
                ...
                <div class="flex justify-between items-start mb-0 pb-1" style="border-bottom: 1px dashed #111827;">
                  <div class="flex-1">
                    <h3 class="text-lg font-bold text-gray-900 mb-1">
                      ...
                      <span v-if="quota.pay && quota.pay_status === 1"
                        class="absolute top-2 right-2 bg-warning text-white text-xs font-bold px-2 py-1 rounded-md">
                ...
                <div class="flex justify-center items-center flex-wrap gap-2">
                  <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-white">
                      Total S/. {{ Number(quota.amount).toFixed(2) }}
                    </span>
                  </div>
                  <q-btn v-if="quota.pending_validation" unelevated rounded color="white" text-color="warning" size="sm"
                    label="Validar pago" class="validatePayQoutaButton" @click="goToValidate(quota, $event)" />
                </div>
              </div>
            </div>
          </div>
